...
class LMPC(object):
	...
	def __init__(self, ftocp, l, P, verbose, Xf_vertices = None):
		self.ftocp = ftocp
		self.SS    = []
		self.uSS   = []
		self.Qfun  = []
	...
	def solve(self, xt, verbose = 5):		
		...
		for i in range(minIt, self.it):
			...
			if (self.Xf_vertices is not None) and (idx[-1] == np.shape(self.SS[i])[1]-1):
				...
				augCost = np.concatenate((self.Qfun[i][idx], np.zeros(self.Xf_vertices.shape[1])), axis=0 )	
				SSQfun.append( np.concatenate( (augSS, [augCost]), axis=0 ).T )
				...
				verticesAndInputs = np.concatenate((self.Xf_vertices, np.zeros((self.ftocp.d, self.Xf_vertices.shape[1]))), axis=0)
